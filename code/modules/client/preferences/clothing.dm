/proc/generate_underwear_icon(datum/sprite_accessory/accessory, icon/base_icon, color)
	...
	if (!isnull(accessory))
		var/icon/accessory_icon = icon(accessory.icon, accessory.icon_state) // EffigyEdit Change: Original - var/icon/accessory_icon = icon('icons/mob/clothing/underwear.dmi', accessory.icon_state)
...
/datum/preference/choiced/undershirt/icon_for(value)
	...
	if (value != "Nude")
		...
		icon_with_undershirt.Blend(icon(accessory.icon, accessory.icon_state), ICON_OVERLAY) // EffigyEdit Change - Original - icon_with_undershirt.Blend(icon('icons/mob/clothing/underwear.dmi', accessory.icon_state), ICON_OVERLAY)
